...
To test the installation, run the following. If you see the help dialog, you're good to go!
```{bash, eval=FALSE, engine='sh'}
wine ./.wine/drive_c/Program\ Files/Innovasea/Fathom\ Connect/vdat.exe --help
```

`rvdat` should detect the location of the vdat executable within the Wine installation. Confrim by running `rvdat::vdat_here()`. 
...
vdat_to_csv(
  list.files(td, pattern = "HR2.*\\.vdat$", full.names = T),
  outdir = td
)
```
